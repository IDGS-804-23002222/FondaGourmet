...
def registrar():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            current_app.logger.info(f"Empleado registrado exitosamente: {persona.nombre} {persona.apellido_p} ({usuario.username}) con rol {usuario.id_rol}")
...
def editar(id_empleado):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error al actualizar el empleado: {str(e)}")
...
def eliminar(id_empleado):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error al desactivar el empleado: {str(e)}")
...
def activar(id_empleado):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error al activar el empleado: {str(e)}") 
        flash(f'Error: {str(e)}', 'danger')
